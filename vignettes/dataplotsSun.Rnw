...
<<setup-function>>=
lamp.plotter <- function(lamp.name, w.low=250.0, w.high=900, scaled="peak"){
w.length.out <- seq(from=w.low, to=w.high, length.out=300)
spectrum.data <- calc_lamp_output(w.length.out=w.length.out, lamp.name=lamp.name, scaled=scaled)
...
for (spc in spectra) {
  lamp.plotter(lamp.name=spc, w.low=200, w.high=4000)
}
@
