...
viikki_field.geo <- data.frame(lon = 25.019212, lat = 60.226805, address = "Viikki, Helsinki, Finland")
...
where_measured(irrad_Kipp.data) <- viikki_field.geo
...
where_measured(ppfd_LICOR.data) <- viikki_field.geo
...
where_measured(ppfd_BF.data) <- viikki_field.geo
...
where_measured(sun_four_days_1min.tb) <- viikki_field.geo
